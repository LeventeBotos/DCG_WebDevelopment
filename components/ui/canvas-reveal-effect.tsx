...
type Uniforms = Record<string, UniformSpec>;
...
const ShaderMaterialMesh = ({
  ...
  const preparedUniforms = useMemo(() => {
    ...
    for (const name of Object.keys(uniforms)) {
      ...
      if (u.type === "uniform1fv") out[name] = { value: u.value };
  ...
  useEffect(() => {
    const m = material;
    const u = m.uniforms?.u_resolution?.value as THREE.Vector2 | undefined;
    if (!u) return;
    u.set(size.width * 2, size.height * 2);
  ...
  useFrame((_state, delta) => {
    ...
    if (frameBudgetRef.current < 1 / maxFps) return;
    frameBudgetRef.current = 0;

    const mat = mesh.material;
    const uTime = mat.uniforms?.u_time;
    if (uTime) uTime.value = elapsedRef.current;
  });
